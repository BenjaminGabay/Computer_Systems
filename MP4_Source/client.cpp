...
int main(int argc, char *argv[])
{
    if (argc != 4) //Valid number of command line arguments
    {
        ...
        std::cout << "\t-n: <number of data requests per patient>" << endl;
        std::cout << "\t-b: <size of bounded buffer between request and worker threads>" << endl;
        std::cout << "\t-w: <number of request channels to be handled by event handler thread>" << endl;
        return 1;
    }
